...
public class ProductChildrenService {
    ...
    public PageResponse<List<ProductChildrenResponse>> getAllProductChildren(String keyword,
        ...
        var result =
                productChildrenRepository.findAllProductChildren(kw, pageSize, offset, productId);
        long total = productChildrenRepository.countProductChildren(kw, productId);

        var data = result.stream().map(r -> (ProductChildrenResponse) ProductChildrenResponse
                .builder().id(r.getId()).name(r.getName()).price(r.getPrice())
                .colorCode(r.getColorCode()).size(r.getSize()).quantity(r.getQuantity())
