...
public class ProductService {
        ...
        public PageResponse<List<ProductResponse>> getProducts(String keyword, int pageNumber, int pageSize) {
                int offset = (pageNumber - 1) * pageSize;
                String kw = keyword == null ? "" : keyword;

                List<ProductProjection> projections = productRepository.findAllProducts(kw, pageSize, offset);
                long total = productRepository.countProducts(kw);

                List<ProductResponse> data = projections.stream()
                                .map(p -> (ProductResponse) ProductResponse.builder()
                                                .id(p.getId())
                                                .name(p.getName())
                                                .description(p.getDescription())
                                                .price(p.getPrice())
                                                .imageUrl(p.getImageUrls() != null
        ...
        public PageResponse<List<ProductStockResponse>> getProductsStock(String keyword, int pageNumber, int pageSize) {
                int offset = (pageNumber - 1) * pageSize;
                String kw = keyword == null ? "" : keyword;
                List<ProductStockProjection> projections = productRepository.findProductStocks(kw, pageSize, offset);
                long total = productRepository.countProducts(kw);

                List<ProductStockResponse> data = projections.stream()
                                .map(p -> (ProductStockResponse) ProductStockResponse.builder()
                                                .id(p.getId())
                                                .name(p.getName())
